...
DreamShader *DreamGLGraphics::LoadShader(const wchar_t *file, ShaderType shaderType)
{
	...
	auto shaderCodeLength = DreamFileIO::ReadFullFileWithoutMakingPandaCry(path.c_str(), &shaderCode);
	...
	auto safeLength = shaderCodeLength % 4 ? (shaderCodeLength + shaderCodeLength % 4) : shaderCodeLength;
	if (safeLength != shaderCodeLength)
	{
		printf("Resizing for safety %s != %s\n", safeLength, shaderCodeLength);
	}
	char *safeShaderCode = safeLength == shaderCodeLength ? shaderCode : new char[safeLength];

	memcpy(safeShaderCode, shaderCode, shaderCodeLength);

	uint32_t *code = reinterpret_cast<uint32_t *>(shaderCode);

	// Read SPIR-V from disk or similar.
	std::vector<uint32_t> spirv_binary;
	spirv_cross::CompilerGLSL glsl(code, safeLength / sizeof(uint32_t));
