...
#pragma region Operator Overload
		...
		DreamMatrix3X3 operator*(DreamMatrix3X3 m)
		{
			...
			for (int i = 0; i < (3 * 3); i++)
			{
				int curRow = i / 3;
				int curCol = i % 3;
				newMatrix.matrix.data[curRow][curCol] = (this->matrix.data[curRow][0] * m.matrix.data[0][curCol]) + (this->matrix.data[curRow][1] * m.matrix.data[1][curCol]) + (this->matrix.data[curRow][2] * m.matrix.data[2][curCol]);
			}
...
#pragma region Operator Overloads
		...
		DreamMatrix4X4 operator*(DreamMatrix4X4 m)
		{
			...
			for (int i = 0; i < (4 * 4); i++)
			{
				int curRow = i / 4;
				int curCol = i % 4;
				newMatrix.matrix.data[curRow][curCol] = (this->matrix.data[curRow][0] * m.matrix.data[0][curCol]) + (this->matrix.data[curRow][1] * m.matrix.data[1][curCol]) + (this->matrix.data[curRow][2] * m.matrix.data[2][curCol]) + (this->matrix.data[curRow][3] * m.matrix.data[3][curCol]);
			}
			return newMatrix;
		}
...
#pragma endregion
	...
	public:
		...
		DreamMatrix4X4 GetWorldMatrix()
		{
			...
			DreamMatrix4X4 worldMat = ((DreamMath::CreateScaleMatrix(scale) * DreamMath::CreateRotationMatrix(rotation)) * DreamMath::CreateTranslationMatrix(position));
			return worldMat;
		}
	...
	static DreamMatrix4X4 LookAtViewMatix(DreamVector3 eye, DreamVector3 target, DreamVector3 up)
	{
		...
		lookAtMat.matrix.xRow = DreamVector4{xaxis.x, xaxis.y, xaxis.z, 0.0f};
		lookAtMat.matrix.yRow = DreamVector4{yaxis.x, yaxis.y, yaxis.z, 0.0f};
		lookAtMat.matrix.zRow = DreamVector4{zaxis.x, zaxis.y, zaxis.z, 0.0f};
		lookAtMat.Transpose();
		lookAtMat.matrix.wRow = DreamVector4{waxis.x, waxis.y, waxis.z, 1.0f};
